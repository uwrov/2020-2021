...
class Settings extends React.Component {
  state = {
    ip: "localhost",
    ports: [{ value: "8080" }, { value: "8081" }, { value: "8082" }],
    left: 100,
    top: 100,
    ip_only: false,
  };
  ...
  render() {
    return (
      <div id="settings" style={this.getStyles()}>
        <div class="header">
          <div onClick={this.handleExit}>&times;</div>
        </div>
        {this.renderCamIp()}
        <br />
        Ip-only Mode:
        ...
        <div>
          {this.renderPorts()}
          <div className="button" onClick={() => this.addPort()}>
            Add Port
          </div>
        </div>
  ...
  portLabel(index) {
    if (!this.state.ip_only) return "Port " + (index + 1) + ": ";
    return "IP " + (index + 1) + ": ";
  }
  ...
  removePort(index) {
    ...
    if (index < newPorts.length && index >= 0) newPorts.splice(index, 1);
